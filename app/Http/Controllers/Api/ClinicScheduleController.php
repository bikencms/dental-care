...
class ClinicScheduleController extends Controller
{
    ...
    /**
     * API Lấy cấu hình lịch theo clinic_id và service_type
     */
    public function getScheduleByService(Request $request, $id)
    {
        $serviceType = $request->query('service_type');

        // Lấy tất cả lịch cố định theo clinic_id và service_type
        $schedules = ClinicSchedule::where('clinic_id', $id)
            ->where('service_type', $serviceType)
            ->get()
            ->keyBy('day_of_week');

        // Nếu đã có dữ liệu lưu trước đó
        if ($schedules->isNotEmpty()) {
            $first = $schedules->first();
            $slotDuration = $first->slot_duration_minutes;
            $maxPatients = $first->max_patients_per_slot;
        } else {
            // Giá trị mặc định nếu service này chưa từng được cài đặt
            $slotDuration = 30;
            $maxPatients = 1;
        }

        // Format lại dữ liệu theo từng ngày trong tuần (0-6)
        $formattedSchedules = [];
        for ($day = 0; $day <= 6; $day++) {
            if (isset($schedules[$day])) {
                ...
                // Mặc định nếu ngày đó chưa có cấu hình
                $formattedSchedules[$day] = [
                    'is_active'  => true,
                    'start_time' => '07:00',
                    'end_time'   => '19:00',
